...
*/
;(function() {
var CORDOVA_JS_BUILD_LABEL = '2.7.0-w2-0.6-0-g5b3530b';
...
var coreModules = {
    ...
    "Battery": require('cordova/plugin/webos/battery'),
    "Globalization": require('cordova/plugin/webos/globalization'),
    "Button": require('cordova/plugin/webos/button')
};
    ...
    start: function(onSuccess, onFailure) {
        if (!isLegacy) {
            if (typeof(service) != null) {
                request = service.Request('luna://com.webos.sensord', {
                    method: "getValue",
                    parameters: {
                        sensor: "Accel",
                        subscribe: true
                    ...
                    onSuccess: function(inResponse) {
                        if(typeof(inResponse.data.x) === "number"
                            && typeof(inResponse.data.y) === "number"
                            && typeof(inResponse.data.z) === "number") {
                            onSuccess({x:(inResponse.data.x), y:(inResponse.data.y), z:(inResponse.data.z)});
                    }
                },
                onFailure: function(inError) {
                    onFailure({code:inError.errorCode || -1, message:inError.errorText || "Unknown Acceloremeter error"});
                }
                });
            }
...
// file: lib/webos/plugin/webos/battery.js
define("cordova/plugin/webos/battery", function(require, exports, module) {
...
module.exports = {
    start: function (successCallback, errorCallback, args) {
        ...
        // events using powerd service API.
        request = service.Request('palm://com.palm.bus/signal/', {
            method: 'addmatch',
            subscribe:true,
            ...
            onSuccess: function(response) {
                ...
                var level = response.percent_ui;

                successCallback({'level' : level, 'isPlugged' : me._isPlugged});
            },
        ...
        request = service.Request('palm://com.palm.bus/signal/', {
            ...
            onSuccess: function(response) {
                var me = navigator.battery;
                var isConnected = response.connected;

                // state is either CHARGING or UNPLUGGED
                if (response) {
                    var info = {
                        "level" : me._level,
                        "isPlugged" : isConnected === true
                    };
...
// file: lib/webos/plugin/webos/button.js
define("cordova/plugin/webos/button", function(require, exports, module) {
...
module.exports = {
    start: function(successCallback, errorCallback, args) {
        if (typeof(service) != null) {
            // Use service to get key pressed and state of button
            request = service.Request('palm://com.webos.keys/', {
                method: 'keyStatus',
                subscribe: true,
                onSuccess: function(inResponse) {
                    if (inResponse.state === "release"
                        && inResponse.key === "back") {
                        successCallback(inResponse.key);
                }
            }
        });
        }
    ...
    stop: function(successCallback, errorCallback, args) {
        // Unregister event button listeners.
...
// The compass sensor maximum and minimum limits
...
module.exports = {
    getHeading: function(onSuccess, onFailure) {
        ...
        if(window.device.name !== "TouchPad" && window.device.name !== "Pre3" && window.device.name !== "webOS Device" && onFailure) {
            onFailure(CompassError.COMPASS_NOT_SUPPORTED);
        } else if(window.device.name === "webOS Device") {
            var mx,my,mz,arctan,azimuth;
            if (typeof(service) !== null) {
                var request = service.Request('luna://com.webos.sensord', {
                    method: "getValue",
                    parameters: {
                        sensor: "Mag",
                        subscribe: true
                    ...
                    onSuccess: function(inResponse) {
                        if(typeof(inResponse.data.x) === "number"
                            && typeof(inResponse.data.y) === "number"
                            && typeof(inResponse.data.z) === "number") {
                            mx = inResponse.data.x;
                            my = inResponse.data.y;
                            mz = inResponse.data.z;
                            ...
                            var xsf1 = (ymax - ymin)/(xmax - xmin);
                            var xsf = xsf1 > 1 ? xsf1 : 1;
                            var ysf1 =(xmax - xmin)/(ymax - ymin);
                            var ysf = ysf1 > 1 ? ysf1 : 1;
                            var xoff = ((xmax + xmin)/2 )* (xsf);
                            var yoff = ((ymax + ymin)/2 )* (ysf);

                            var xh = mx * xsf - xoff;
                            var yh = my * ysf - yoff;

                            //convert to degree
                            arctan = (Math.atan2(yh,xh) * (180/Math.PI));

                            azimuth = (arctan === 0) ? arctan : (arctan < 0 ? (arctan + 360) : arctan);
                            ...
                            onSuccess({magneticHeading:(azimuth), trueHeading:(0), headingAccuracy:(0)});
                    }
                },
                onFailure: function(inError) {
                    onFailure(CompassError.COMPASS_INTERNAL_ERR);
                }
            });
            }
        }
...
//        var request = this.requests[requests.length] = service.request('palm://com.palm.location', {
        service.request('palm://com.palm.location', {
            method: "getCurrentPosition",
            parameters: {
                accuracy: ((options[0]==true) ? 2 : 3),
                    ...
                    onSuccess: function(inResponse) {
                        ...
                        for (var i = 0, max = locale.length; i < max; i++) {
                            if (locale[i].languageCode == languageCode) {
                                successCallback(JSON.parse('{"value":'+'"'+locale[i].languageName+'"'+'}'));
                            }
                        }
                    },
    ...
    getLocaleName: function(successCallback, errorCallback) {
        service.request('luna://com.palm.systemservice', {
            ...
            onSuccess: function(inResponse) {
                ...
                successCallback(JSON.parse('{"value":'+'"'+locale+'"'+'}'));
            },
            onFailure: errorCallback
        });
    }
    ...
    if(!navigator.connectionMonitor.request) {
        navigator.connectionMonitor.request = service.Request('luna://com.palm.connectionmanager', {
            ...
            onSuccess: function(result) {
                ...
                if(result.wan && result.wan.internet && result.wan.internet.state === "connected") { type = Connection.CELL; }
                ...
                if(type != Connection.CELL) {
                    if(type != navigator.connection.type)
                        onSuccess(type);
                }
                else{
                    request = service.Request('palm://com.palm.telephony', {
                    method:'networkStatusQuery',
                    onSuccess: function(response) {
                        var inresponse = response.extended.dataType;
                        if(inresponse === "gprs" || inresponse === "edge") { type = Connection.CELL_2G; }
                        if(inresponse === "hsdpa" || inresponse === "umts" || inresponse === "hspa") { type = Connection.CELL_3G; }
                        if(inresponse === "lte") { type = Connection.CELL_4G; }
...
module.exports = {
    ...
    vibrate: function(onSuccess, onFailure, args) {
        if(!isLegacy) {
            var service = require('cordova/plugin/webos/service');
            this.vibRequest = service.request("luna://com.webos.vibrate", {
                method: 'vibrate',
                parameters: {
                    period: 0,
                    duration: args[0],
    ...
    beep: function(onSuccess, onFailure, args) {
        if(!isLegacy) {
            var service = require('cordova/plugin/webos/service');
            var beepCount = args[0];
            function interval() {
                if(beepCount > 0) {
                    var timerId = setInterval(function() {interval()}, 1000);
                    beepCount--;
                    var beepRequest = service.Request("luna://com.palm.audio/systemsounds", {
                        method: 'playFeedback',
                        parameters: {
                            name: "alert_0",
                            sink:"pfeedback",
            ...
            setTimeout(function() {interval()}, 0);
        } else {
            PalmSystem.playSoundNotification('alerts');
        }
    }
